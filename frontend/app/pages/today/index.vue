...
const allDueToday = computed(() => store.getTasksDueToday)
const availableTags = computed(() => getUniqueTags(allDueToday.value))

const effectiveSelectedTag = computed(() => {
  return availableTags.value.includes(selectedTag.value) ? selectedTag.value : null
})

const filteredTasks = computed(() => filterByTag(allDueToday.value, effectiveSelectedTag.value))
...
<template>
  <div class="flex flex-col gap-6 p-4 sm:p-8 max-w-5xl mx-auto w-full">
    ...
    <div class="card">
      <template v-if="allDueToday.length === 0">
      ...
      <template v-else>
        ...
        >
          ...
          <span v-if="task.tags.length > 0" class="task-tag">
            {{ task.tags[0].name }}
          </span>
          <span v-else></span>
